...
USAGE_MESSAGE = "[-h, -?, --help]&[--version]&[-e, --errors]&[-w, --warnings]&[-i, --info]&[-d, --debug]&[-v, --verbose]&[-vv, --veryverbose]&[-a TIMESTAMP, --after TIMESTAMP]&[-b TIMESTAMP, --before TIMESTAMP]&[-t TAG_NAME, --tag TAG_NAME]&[-c STATEMENT_ID | -q EVENT_ID]&[-f, --follow]&[-s, --status]&(-x, --stdin) | <QUEUE>"

COMMAND_INFO = """Use the lognestmonster comand to parse files generated by the lognestmonster library.



By default, inputting a queue file, a folder with queue files, or switching stdin (-x, --stdin) will display an interactive textual interface where keyboard events are used to sift through log statements. Using the status switch (-s, --status), you can do a quick read of the instream and only display metadata, such as the number of statements or events. If the follow switch is used (-f, --follow), the parser will keep the instreams open and generate live logtrees. See other switches and options for the ability to narrow down log results.
...
DESCRIPTION_STDIN = "Reads logtree data from stdin as compared to logs from a log directory. Implicitly uses --tail"
